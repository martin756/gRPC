...
CREATE TABLE IF NOT EXISTS `retroshop`.`usuario` (
  ...
  `esmonitor` TINYINT NOT NULL DEFAULT '0',
...
CREATE TABLE IF NOT EXISTS `retroshop`.`factura` (
  ...
  INDEX `fk_factura_producto_carrito_idx` (`idcarrito` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `retroshop`.`producto` (
  ...
  `esSubasta` TINYINT NOT NULL DEFAULT '0',
...
CREATE TABLE IF NOT EXISTS `retroshop`.`producto_carrito` (
  ...
  INDEX `fk_producto_carrito_factura1_idx` (`idfactura` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_producto_carrito_factura1`
    FOREIGN KEY (`idfactura`)
    REFERENCES `retroshop`.`factura` (`idfactura`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
...
CREATE TABLE IF NOT EXISTS `retroshop`.`subasta` (
  ...
  `pujador_idusuario` INT NULL DEFAULT NULL,
  PRIMARY KEY (`idsubasta`),
  INDEX `fk_idproducto_idx` (`idproducto` ASC) VISIBLE,
  INDEX `fk_pujador_idusuario` (`pujador_idusuario` ASC) VISIBLE,
