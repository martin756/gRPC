...
function MyShopping() {
  ...
  const idusuario =
    cookies.get("Idusuario") === undefined ? 0 : cookies.get("Idusuario");
    ...
    await axios
      ...
      .then((response) => {
        ...
        let obj = response.data.reduce((res, curr) => {
          if (res[curr.Idcarrito]) res[curr.Idcarrito].push(curr);
          else Object.assign(res, { [curr.Idcarrito]: [curr] });
          return res;
        }, {});
        ...
        arr.forEach((subArray) => {
          obj = subArray.reduce((res, curr) => {
            ...
            else Object.assign(res, { [curr.DatosFactura.Idfactura]: [curr] });
            return res;
          }, {});
  ...
  var url = "http://localhost:8080/factura/";
  return (
    <div>
      <Header />
      <div
        className="d-flex g-5 justify-content-center"
        style={{ padding: "30px" }}
      >
            ...
            <div>
              {carritos.map((value) => (
                <div className="row list-group mb-3">
                  <h4>Carrito nro. {value[0][0].Idcarrito}</h4>
                  {value.map((carrito) => (
                    <div>
                      <h5 class={carrito[0].DatosFactura.Idfactura}>
                        Factura nro. {carrito[0].DatosFactura.Idfactura}
                      </h5>

                      <a href="http://localhost:8080/factura/1">
                        generar factura
                      </a>
                      <form action="http://localhost:8080/factura/2">
                        <input
